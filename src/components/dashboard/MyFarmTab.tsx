  ...
  const renderMapContent = () => {
    ...
    if (locationContext?.loading) {
        return (
            <div className="flex items-center justify-center h-full">
                <Loader2 className="h-6 w-6 animate-spin text-primary"/>
                <p className="ml-2">Finding Location...</p>
            </div>
        );
    }
    
    if (locationContext?.error || !locationContext?.coordinates) {
       return (
        <div className="flex flex-col items-center justify-center text-center p-4 bg-muted h-full rounded-md">
    ...
    return <MapComponent apiKey={apiKey!} coordinates={locationContext.coordinates} />;
  }
